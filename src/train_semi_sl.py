...
from medAI.datasets.data_bk import make_semi_sl_bk_dataloaders
...
class SemiSLExperiment:
    ...
    def setup(self):
        ...
        self.setup_model()
        if self.config.training.from_ckpt is not None:
            self.model.load_state_dict(
                torch.load(f'{self.config.training.from_ckpt}/fold{self.config.data.fold}.pth')
            )
    ...
    def run_train_epoch(self, loader, desc="train"):
        ...
        for train_iter, batch in enumerate(tqdm(loader, desc=desc)):
            ...
            bmode1, bmode2, needle_mask, prostate_mask, ood_mask, label, *metadata = batch
            
            bmode1 = bmode1.unsqueeze(1)
            bmode2 = bmode2.unsqueeze(1)

            prostate_mask = prostate_mask.unsqueeze(1)
            needle_mask = needle_mask.unsqueeze(1)
            ood_mask = ood_mask.unsqueeze(1)
            
            bmode1 = torch.cat([bmode1, bmode1, bmode1], dim=1) / bmode1.max()
            bmode2 = torch.cat([bmode2, bmode2, bmode2], dim=1) / bmode2.max()
            bmode1 = bmode1.to(self.config.device)
            bmode2 = bmode2.to(self.config.device)

            needle_mask = needle_mask.to(self.config.device)
            prostate_mask = prostate_mask.to(self.config.device)
            ood_mask = ood_mask.to(self.config.device)
            ...
            with torch.cuda.amp.autocast(enabled=self.config.use_amp):
                # forward pass
                heatmap_logits_1 = self.model(
                    bmode1,
                    prostate_mask=prostate_mask,
                    needle_mask=needle_mask
                )

                heatmap_logits_2 = self.model(
                    bmode2,
                    prostate_mask=prostate_mask,
                    needle_mask=needle_mask
                )

                if torch.any(torch.isnan(heatmap_logits_1)):
                ...
                preds1, b1 = MaskedPredictionModule()(heatmap_logits_1, masks)
                preds2, b2 = MaskedPredictionModule()(heatmap_logits_2, masks)

                meanpreds1 = []
                meanpreds2 = []
                for j in range(B):
                    meanpreds1.append(
                        preds1[b1 == j].sigmoid().mean()
                    )
                    meanpreds2.append(
                        preds2[b2 == j].sigmoid().mean()
                    )
                meanpreds1 = torch.stack(meanpreds1)
                meanpreds2 = torch.stack(meanpreds2)
                mean_predictions_in_needle = (meanpreds1 + meanpreds2) / 2
    ...
    def show_example(self, batch):
        ...
        preds_1 = logits_1.sigmoid()
        preds_2 = logits_2.sigmoid()

        logits_1 = logits_1.cpu().double()
        logits_2 = logits_2.cpu().double()
        
        preds_1 = preds_1.cpu().double()
        preds_2 = preds_2.cpu().double()
        
        label = label.cpu().double()

        image1 = bmode1.cpu().double()
        image2 = bmode2.cpu().double()
        ...
        ax[1].imshow(image2[0].permute(1, 2, 0), cmap='gray', **kwargs)
        ...
        ax[3].imshow(image2[0].permute(1, 2, 0), cmap='gray', **kwargs)
        ax[3].imshow(preds_2[0, 0], alpha=0.5, cmap='jet', **kwargs)
        ax[3].contour(prostate_mask[0, 0], colors='#ffffff', **kwargs)
        ax[3].contour(needle_mask[0, 0], colors='#00eeff', **kwargs)
        masked_pred = (needle_mask * prostate_mask * preds_2[0, 0])
        ax[3].set_title(f"y={label[0].item()} pred={masked_pred.mean().item():.2f}")
